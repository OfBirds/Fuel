...
Fuel ships its **user-facing** docs as an Antora component under `docs/` — component name **`swallow`**, title "Indigo Swallow": `docs/antora.yml`, pages in `docs/modules/ROOT/pages/` (what-it-is / self-hosting / configuration), nav in `docs/modules/ROOT/nav.adoc`, an example compose in `docs/modules/ROOT/examples/`. These are distinct from the maintainer-facing `docs/*.md` design notes, which Antora ignores. Copy the shape from ThoseDaysApp (`rosella`).

The flock's docs site (the **OfbirdsWeb** repo → ofbirds.org/docs) builds these with Antora and pulls Fuel's docs **by branch, per deploy lane**:
