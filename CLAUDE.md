...
**Configuration is flat env vars, not nested config sections.** `Program.cs` reads `DB_HOST`/`DB_PORT`/`DB_NAME`/`DB_USER`/`DB_PASSWORD` and `SMTP_*` directly from `builder.Configuration[...]` with hardcoded localhost/postgres fallbacks. Follow this `KEY` style for new config rather than `appsettings.json` sections.
